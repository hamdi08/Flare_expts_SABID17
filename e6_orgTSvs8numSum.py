...
dataset = prepare_dataset(lookback=12, span=24, withC=False)
...
preproc_time = stop - start
print('Classification starts with whole time series of TOTUSJH...')
...
orgts_l12_perf = [acc, pr_pos, pr_neg, rc_pos, rc_neg, f1_pos, f1_neg, HSS1, HSS2, GS, TSS]
orgts_l12_time = [preproc_time, train_time, test_time]

print('acc pr_pos pr_neg rc_pos rc_neg f1_pos f1_neg HSS1 HSS2 GS TSS')
print(orgts_l12_perf)
print('train and test time')
print(orgts_l12_time)
#print(measures[0,0, :])
...
stop = timeit.default_timer()
preproc_time = stop - start
print('Classification starts with whole 8 number summary of TOTUSJH...')
print('X shape: ', X.shape)
print('y shape: ', y.shape)
print('Model : knn with k 1')
...
sumts_l12_perf = [acc, pr_pos, pr_neg, rc_pos, rc_neg, f1_pos, f1_neg, HSS1, HSS2, GS, TSS]
sumts_l12_time = [preproc_time, train_time, test_time]

print('acc pr_pos pr_neg rc_pos rc_neg f1_pos f1_neg HSS1 HSS2 GS TSS')
print(sumts_l12_perf)
print('train and test time')
print(sumts_l12_time)
...
rects1 = ax.bar(ind, orgts_l12_perf, width, color='#ff81c0')
rects2 = ax.bar(ind + width, sumts_l12_perf, width, color='#0343df')
ax.set_ylabel('Performance')
ax.set_title('Performance variation with full and summarized time series of TOTUSJH')
...
ax.legend((rects1[0], rects2[0]), ('Full time series', 'Summarized time series'))
...
ax = fig.add_subplot(111)
rects3 = ax.bar(ind, orgts_l12_time[1:3], width, color='#fddc5c') #d6b4fc
rects4 = ax.bar(ind + width, sumts_l12_time[1:3], width, color='#ff474c')
ax.set_ylabel('Time in seconds')
ax.set_title('Runtime variation with full and summarized time series of TOTUSJH')
ax.set_xticks(ind + width / 2)
ax.set_xticklabels(('Training', 'Testing'))
#fig.autofmt_xdate(rotation=90, ha='center')
ax.legend((rects3[0], rects4[0]), ('Full time series', 'Summarized time series'))